...
function resetAIButton() {
  let checkbox = document.getElementById("robot");
...
function generateGame() {
  ...
  for (let row = 1; row <= game_type; row++) {
    ...
    gameBoard.appendChild(breakline);
  }
}
...
function autoTurn(again = false) {
  ...
  for (let x = 0; x < robot_pattern.length; x++) {
    let desired_obj = document.getElementById(robot_pattern[x]);
...
// change gameBoard backgroundColor
let b = document.querySelector(".change-color .colors");
b.addEventListener("click", function changeBg(e) {
  // console.log(e.target.className);
  // console.log(e.target);
  if (e.target.className == "r") {
    gameBoard.style.backgroundColor = "#4C212A";
  } else if (e.target.className == "b") {
    gameBoard.style.backgroundColor = "#01172F";
  } else if (e.target.className == "g") {
    gameBoard.style.backgroundColor = "#00635D";
  }
});
